...
extern PetscErrorCode FormRHSFunctionLocal(
    DMDALocalInfo*, PetscReal, PetscReal**, PetscReal**, HeatCtx*);
extern PetscErrorCode FormRHSJacobianLocal(
    DMDALocalInfo*, PetscReal, PetscReal**, Mat, Mat, HeatCtx*);
...
int main(int argc, char** argv)
{
  ...
  PetscCall(
      PetscOptionsReal("-D0", "constant thermal diffusivity", "heat.c", user.D0, &user.D0, NULL));
  PetscCall(PetscOptionsBool("-monitor",
      "also display total heat energy at each step",
      "heat.c",
      monitorenergy,
      &monitorenergy,
      NULL));
  ...
  PetscCall(DMDACreate2d(PETSC_COMM_WORLD,
      ...
      4,
      PETSC_DECIDE,
      PETSC_DECIDE,  // default to hx=hx=0.25 grid
      1,
      1,  // degrees of freedom, stencil width
      NULL,
      NULL,
      &da));
...
PetscErrorCode Spacings(DMDALocalInfo* info, PetscReal* hx, PetscReal* hy)
{
  ...
  if (hy) {
    *hy = 1.0 / (PetscReal)(info->my);  // periodic direction
  }
  return 0;
}
...
PetscErrorCode EnergyMonitor(TS ts, PetscInt step, PetscReal time, Vec u, void* ctx)
{
  ...
  PetscCall(PetscPrintf(
      PETSC_COMM_WORLD, "  energy = %9.2e     nu = %8.4f\n", energy, user->D0 * dt / (hx * hy)));
  return 0;
}
...
PetscErrorCode FormRHSFunctionLocal(
    DMDALocalInfo* info, PetscReal t, PetscReal** au, PetscReal** aG, HeatCtx* user)
{
...
PetscErrorCode FormRHSJacobianLocal(
    DMDALocalInfo* info, PetscReal t, PetscReal** au, Mat J, Mat P, HeatCtx* user)
{
  PetscInt i, j, ncols;
