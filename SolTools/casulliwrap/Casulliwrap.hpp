...
class CasulliWrap final
  : public SolTool<Real, PDEOptions, CasulliError, CasulliMethodProps<Real>, CasulliAlgOpts>
{
public:
  ...
  auto MethodProps() noexcept -> CasulliMethodProps<Real>& { return this->m_methodProps; }
  auto MethodProps() const noexcept -> CasulliMethodProps<Real> const& { return this->m_methodProps; }

  // ---- Newton logging control ----
  void SetNewtonVerbose(bool on) noexcept { m_newtonVerbose = on; }

  // ---- Optional non-uniform initial conditions ----
  // 1D: eta at cell centers, u at faces
  void SetInitialEta1D(std::function<Real(std::size_t i, Real x)> f) { m_eta_ic_1d = std::move(f); }
  void SetInitialUFace1D(std::function<Real(std::size_t f, Real xf)> g) { m_u_ic_1d = std::move(g); }

#if defined(CASE_2D)
  // 2D: eta at cell centers (flattened i+nx*j)
  void SetInitialEta2D(std::function<Real(std::size_t i, std::size_t j, Real x, Real y)> f) { m_eta_ic_2d = std::move(f); }
#endif

  auto InitSolMethod() -> CasulliError override { return 0; }

  auto InitProblem() -> CasulliError override
  {
    if (!this->m_myPDE) return 1;
    return 0;
  }

  auto SetSolOpts() -> CasulliError override { return 0; }

  auto NumericalSolve() -> CasulliError override
  {
    if (!this->m_myPDE) return 2;
  ...
  auto EvaluateSolution(std::function<std::vector<double>(std::vector<Real>)>&) -> CasulliError override
  {
    return 0;
  }
  ...
  // ------------------------------------------------------------
  auto Eta1D() const -> std::vector<Real> const& { return m_eta1d; }
  auto UFace1D() const -> std::vector<Real> const& { return m_uface1d; }

  auto Eta2D() const -> std::vector<Real> const& { return m_eta2d; }
  auto UFace2D() const -> std::vector<Real> const& { return m_uface2d; }
  auto VFace2D() const -> std::vector<Real> const& { return m_vface2d; }

  void SetGrid1D(std::size_t nx) { m_nx = nx; }
  void SetGrid2D(std::size_t nx, std::size_t ny) { m_nx = nx; m_ny = ny; }

private:
  static auto Idx_(std::size_t i) -> std::size_t { return i; }
  static auto Idx_(std::size_t i, std::size_t j, std::size_t nx) -> std::size_t { return i + nx * j; }

  static auto NormInf_(std::vector<Real> const& v) -> Real
  {
    Real m = Real(0);
    for (auto const& x : v) m = std::max(m, std::abs(x));
    return m;
  }

  static void ThomasSolve_(std::vector<Real> a,
                           std::vector<Real> b,
                           std::vector<Real> c,
  ...
  void Solve1D_(fcfd::pdemodel::SVEcas<Real, PDEOptions>& pde)
  {
    ...
    if (m_nx < 3) throw std::logic_error("CasulliWrap 1D: nx must be >= 3");
    const Real dt = this->m_methodProps.dt;
    const Real dx = this->m_methodProps.dx;
    if (!(dt > Real(0) && dx > Real(0))) throw std::logic_error("CasulliWrap 1D: dt/dx must be > 0");
    ...
    for (int step = 0; step < this->m_methodProps.nSteps; ++step) {
      ...
      for (std::size_t f = 0; f < m_nx - 1; ++f) {
        ...
        u_star[f] = ui * (Real(1) - (dt / dx) * (ui - uim))
                  - mp.g * (dt / dx) * (m_eta1d[f + 1] - m_eta1d[f]);
      }

      // face depths
      std::vector<Real> Hf(m_nx - 1, Real(0));
      for (std::size_t f = 0; f < m_nx - 1; ++f) Hf[f] = Real(0.5) * (Hc[f] + Hc[f + 1]);
      ...
      for (std::size_t f = 0; f < m_nx - 1; ++f) {
        ...
        if (Hf[f] <= this->m_methodProps.dryTol || denom <= this->m_methodProps.dryTol) alpha[f] = Real(0);
        else alpha[f] = (Hf[f] * Hf[f]) / denom;
      }
      ...
      for (std::size_t i = 0; i < m_nx; ++i) {
        ...
        if (i < m_nx - 1) Fp = HG_over_interior_face(i);
        else              Fp = HG_over_right_boundary();

        if (i > 0)        Fm = HG_over_interior_face(i - 1);
        else              Fm = HG_over_left_boundary();
      ...
      auto enforce_lower_bound = [&](){
        for (std::size_t i = 0; i < m_nx; ++i) {
          ...
          if (z[i] <= -h) z[i] = -h + Real(1e-8);
        }
      };

      enforce_lower_bound();

      if (left_is_eta)  z[0]      = etaL;
      if (right_is_eta) z[m_nx-1] = etaR;
      ...
      for (int it = 0; it < this->m_methodProps.newtonMaxIts; ++it) {
        ...
        for (std::size_t i = 0; i < m_nx; ++i) {
          const Real h = pde.BottomAt(Real(i) * dx);
          const Real s = h + z[i];
          if (s > Real(0)) { Hz[i] = s; Ddiag[i] = Real(1); }
          else             { Hz[i] = Real(0); Ddiag[i] = Real(0); }
        }

        std::vector<Real> r(m_nx, Real(0));
        for (std::size_t i = 0; i < m_nx; ++i) {
          Real Tz = d[i] * z[i];
          if (i > 0)        Tz += a[i - 1] * z[i - 1];
          if (i + 1 < m_nx) Tz += c[i] * z[i + 1];
          r[i] = Hz[i] + Tz - b[i];
        }

        // Dirichlet residuals
        if (left_is_eta)  r[0]      = z[0]      - etaL;
        if (right_is_eta) r[m_nx-1] = z[m_nx-1] - etaR;

        const Real rinf = NormInf_(r);
        if (m_newtonVerbose) {
          std::cout << "[1D step " << step << "] Newton it=" << it
                    << "  ||r||_inf=" << std::scientific << std::setprecision(6) << rinf
                    << std::defaultfloat << "\n";
        }
        ...
        for (std::size_t i = 0; i < m_nx; ++i) dd[i] += Ddiag[i];
        ...
        if (left_is_eta) {
          dd[0] = Real(1);
          if (m_nx > 1) cc[0] = Real(0);
        }
        if (right_is_eta) {
          dd[m_nx - 1] = Real(1);
          if (m_nx > 1) aa[m_nx - 2] = Real(0);
        }

        std::vector<Real> delta;
        ThomasSolve_(aa, dd, cc, r, delta);

        for (std::size_t i = 0; i < m_nx; ++i) z[i] -= delta[i];
      ...
      for (std::size_t f = 0; f < m_nx - 1; ++f) {
        ...
        const Real rhs_u = Hf[f] * u_star[f]
                         - mp.g * dt * (Hf[f]) * (dEta / dx)
                         + dt * mp.gammat * pde.WindSpeedU();
        m_uface1d[f] = rhs_u / denom;
      }

      if (left_is_u)  m_uface1d[0]        = uL;
      if (right_is_u) m_uface1d[m_nx - 2] = uR;

      for (std::size_t f = 0; f < m_nx - 1; ++f)
        if (Hf[f] <= this->m_methodProps.dryTol) m_uface1d[f] = Real(0);
    }
  }
#endif // CASE_1D
  ...
  void Solve2D_(fcfd::pdemodel::SWE2dcas<Real, PDEOptions>& pde)
  {
    ...
    if (m_nx < 3 || m_ny < 3) throw std::logic_error("CasulliWrap 2D: nx,ny must be >= 3");
    ...
    for (int step = 0; step < this->m_methodProps.nSteps; ++step) {
      ...
      for (std::size_t k = 0; k < Hx.size(); ++k) {
        ...
        ax[k] = (Hx[k] > this->m_methodProps.dryTol && denom > this->m_methodProps.dryTol) ? (Hx[k] * Hx[k] / denom) : Real(0);
      }
      for (std::size_t k = 0; k < Hy.size(); ++k) {
        const Real denom = Hy[k] + gamV[k] * dt;
        ay[k] = (Hy[k] > this->m_methodProps.dryTol && denom > this->m_methodProps.dryTol) ? (Hy[k] * Hy[k] / denom) : Real(0);
      }
      ...
      auto HGx_over_interior = [&](std::size_t i, std::size_t j) -> Real {
        ...
        if (Hx[f] <= this->m_methodProps.dryTol || denom <= this->m_methodProps.dryTol) return Real(0);
      ...
      auto HGy_over_interior = [&](std::size_t i, std::size_t j) -> Real {
        ...
        if (Hy[f] <= this->m_methodProps.dryTol || denom <= this->m_methodProps.dryTol) return Real(0);
        const Real G = Hy[f] * v_star[f] + dt * mp.gammat * pde.WindV();
        return (Hy[f] * G) / denom;
      };

      auto HGx_over_left_boundary = [&](std::size_t j) -> Real {
        if (!left_is_vel) return Real(0);
      ...
      auto HGy_over_bottom_boundary = [&](std::size_t i) -> Real {
        if (!bottom_is_vel) return Real(0);
      ...
      for (std::size_t j = 0; j < m_ny; ++j) {
        for (std::size_t i = 0; i < m_nx; ++i) {
          ...
          if (i < m_nx - 1) Fxp = HGx_over_interior(i, j);
          else              Fxp = HGx_over_right_boundary(j);

          if (i > 0)        Fxm = HGx_over_interior(i - 1, j);
          else              Fxm = HGx_over_left_boundary(j);

          Real Fyp = Real(0), Fym = Real(0);
          if (j < m_ny - 1) Fyp = HGy_over_interior(i, j);
          else              Fyp = HGy_over_top_boundary(i);

          if (j > 0)        Fym = HGy_over_interior(i, j - 1);
          else              Fym = HGy_over_bottom_boundary(i);
      ...
      auto is_dirichlet = [&](std::size_t i, std::size_t j) -> bool {
        if (left_is_eta   && i == 0)      return true;
        if (right_is_eta  && i == m_nx-1) return true;
        if (bottom_is_eta && j == 0)      return true;
        if (top_is_eta    && j == m_ny-1) return true;
        return false;
      };
      auto eta_bc = [&](std::size_t i, std::size_t j) -> Real {
        if (left_is_eta   && i == 0)      return etaL;
        if (right_is_eta  && i == m_nx-1) return etaR;
        if (bottom_is_eta && j == 0)      return etaB;
        if (top_is_eta    && j == m_ny-1) return etaT;
        return Real(0);
      };
      ...
      auto apply_T = [&](std::vector<Real> const& x, std::vector<Real>& Tx)
      {
        ...
        for (std::size_t jj = 0; jj < m_ny; ++jj) {
          for (std::size_t ii = 0; ii < m_nx; ++ii) {
            ...
            if (is_dirichlet(ii, jj)) { Tx[Idx_(ii, jj, m_nx)] = Real(0); continue; }
      ...
      auto cg_solve = [&](std::vector<Real> const& Ddiag,
                          std::vector<Real> const& rhs,
                          std::vector<Real>& x)
      {
        x.assign(N, Real(0));
        ...
        Real rr = dot(r, r);
        if (std::sqrt(rr) < this->m_methodProps.cgTol) return;

        for (int it = 0; it < this->m_methodProps.cgMaxIts; ++it) {
          std::vector<Real> Tp;
          apply_T(pvec, Tp);

          for (std::size_t k = 0; k < N; ++k) Ap[k] = Tp[k] + Ddiag[k] * pvec[k];

          const Real denom = std::max(dot(pvec, Ap), Real(1e-30));
          const Real alpha = rr / denom;

          for (std::size_t k = 0; k < N; ++k) x[k] += alpha * pvec[k];
          for (std::size_t k = 0; k < N; ++k) r[k] -= alpha * Ap[k];

          const Real rr_new = dot(r, r);
          if (std::sqrt(rr_new) < this->m_methodProps.cgTol) break;

          const Real beta = rr_new / std::max(rr, Real(1e-30));
          for (std::size_t k = 0; k < N; ++k) pvec[k] = r[k] + beta * pvec[k];
          rr = rr_new;
        }
      };
      ...
      for (int it = 0; it < this->m_methodProps.newtonMaxIts; ++it) {
        ...
        for (std::size_t jj = 0; jj < m_ny; ++jj) {
          for (std::size_t ii = 0; ii < m_nx; ++ii) {
            ...
            if (s > Real(0)) { Hz[Idx_(ii, jj, m_nx)] = s; Ddiag[Idx_(ii, jj, m_nx)] = Real(1); }
            else             { Hz[Idx_(ii, jj, m_nx)] = Real(0); Ddiag[Idx_(ii, jj, m_nx)] = Real(0); }
          }
        }
        ...
        for (std::size_t k = 0; k < N; ++k) r[k] = Hz[k] + Tz[k] - b[k];
        ...
        const Real rinf = NormInf_(r);
        if (m_newtonVerbose) {
          std::cout << "[2D step " << step << "] Newton it=" << it
                    << "  ||r||_inf=" << std::scientific << std::setprecision(6) << rinf
                    << std::defaultfloat << "\n";
        }
        ...
        for (std::size_t k = 0; k < N; ++k) z[k] -= delta[k];
      ...
      for (std::size_t jj = 0; jj < m_ny; ++jj) {
        for (std::size_t ii = 0; ii < m_nx - 1; ++ii) {
          ...
          const Real denom = Hx[f] + gamU[f] * dt;
          if (Hx[f] <= this->m_methodProps.dryTol || denom <= this->m_methodProps.dryTol) { m_uface2d[f] = Real(0); continue; }
      ...
      for (std::size_t jj = 0; jj < m_ny - 1; ++jj) {
        for (std::size_t ii = 0; ii < m_nx; ++ii) {
          ...
          const Real denom = Hy[f] + gamV[f] * dt;
          if (Hy[f] <= this->m_methodProps.dryTol || denom <= this->m_methodProps.dryTol) { m_vface2d[f] = Real(0); continue; }
      ...
      if (left_is_vel)   for (std::size_t jj = 0; jj < m_ny; ++jj) m_uface2d[idxU(0, jj)] = uL;
      if (right_is_vel)  for (std::size_t jj = 0; jj < m_ny; ++jj) m_uface2d[idxU(m_nx - 2, jj)] = uR;
      if (bottom_is_vel) for (std::size_t ii = 0; ii < m_nx; ++ii) m_vface2d[idxV(ii, 0)] = vB;
      if (top_is_vel)    for (std::size_t ii = 0; ii < m_nx; ++ii) m_vface2d[idxV(ii, m_ny - 2)] = vT;

      for (std::size_t k = 0; k < Hx.size(); ++k) if (Hx[k] <= this->m_methodProps.dryTol) m_uface2d[k] = Real(0);
      for (std::size_t k = 0; k < Hy.size(); ++k) if (Hy[k] <= this->m_methodProps.dryTol) m_vface2d[k] = Real(0);
    }
  }
#endif // CASE_2D
