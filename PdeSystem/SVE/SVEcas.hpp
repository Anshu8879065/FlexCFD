...
public:
  ...
  using ModelParamT = fcfd::pdemodel::ModelParams<Real>;
 // matches your ModelParams.hpp
  ...
  // -----------------------
  auto ModelParamsRef() noexcept -> ModelParamT& { return m_modelParams; }
  auto ModelParamsRef() const noexcept -> ModelParamT const& { return m_modelParams; }

  auto GetModelParams() noexcept -> ModelParamT& { return m_modelParams; }
  auto GetModelParams() const noexcept -> ModelParamT const& { return m_modelParams; }

  // Many parts of your codebase expect pde.ModelParams():
  auto ModelParams() noexcept -> ModelParamT& { return m_modelParams; }
  auto ModelParams() const noexcept -> ModelParamT const& { return m_modelParams; }

  // -----------------------
  // user setters (defaults 0)
  // -----------------------
  void SetIC(Real eta0, Real u0) { m_eta0 = eta0; m_u0 = u0; }

  // Bottom: constant depth h0; optional slope S0 (so h(x)=h0-S0*x)
  void SetBed(Real h0, Real S0 = Real(0)) { m_h0 = h0; m_S0 = S0; }

  void SetWindSpeedU(Real ua) { m_ua = ua; }

  void SetBCSpec(SubcriticalBCSpec spec) { m_bc.spec = spec; }
  ...
  auto BC() const noexcept -> CasulliBC1D<Real> const& { return m_bc; }

  auto BottomAt(Real x) const noexcept -> Real { return m_h0 - m_S0 * x; }
  ...
  auto WindSpeedU() const noexcept -> Real { return m_ua; }
  ...
  auto GammaFace(Real H_face, Real u_star) const noexcept -> Real
  {
    ...
    const Real R = (m_modelParams.radi > m_dryTol)
                     ? m_modelParams.radi
                     : std::max(H_face, m_dryTol);
  ...
  auto InitCond() -> Real override { return Real{}; }
...
private:
  ...
  {
    Base::SetBottom([this](std::span<const Real> coords) -> std::vector<Real>
    {
      const Real x = coords.empty() ? Real(0) : coords[0];
      return {BottomAt(x)};
    });

    Base::SetInitCond([this](std::vector<Real> const&) -> std::vector<Real>
    {
    ...
    Base::SetRHSFunc([](std::vector<Real> const&, std::vector<Real>& out, int) -> std::vector<Real>
    { out.assign(2, Real(0)); return out; });

    Base::SetJacRHS([](std::vector<Real> const&, std::vector<Real>& out, int) -> std::vector<Real>
    { out.assign(2, Real(0)); return out; });

    auto lhsa = [](std::vector<Real> const&, std::vector<Real> const& vdot, std::vector<Real> const&,
                   std::optional<std::vector<Real>> const&, std::optional<std::vector<Real>> const&,
                   std::vector<Real>& out) -> std::vector<Real>
    { out = vdot; if (out.size()!=2) out.resize(2, Real(0)); return out; };

    auto lhsb = [](std::vector<Real> const&, std::vector<Real> const&, std::vector<Real> const&,
                   std::optional<std::vector<Real>> const&, std::optional<std::vector<Real>> const&,
                   std::vector<Real>& out) -> std::vector<Real>
    { out.assign(2, Real(0)); return out; };
    ...
    auto jac0 = [](std::vector<Real> const&, std::vector<Real> const&, std::vector<Real> const&,
                   std::optional<std::vector<Real>> const&, std::optional<std::vector<Real>> const&,
                   std::vector<Real>& out, int, int, int) -> std::vector<Real>
    { out.assign(2, Real(0)); return out; };
