...
public:
  ...
  // IC seeds
  void SetIC(Real eta0, Real u0, Real v0) { m_eta0 = eta0; m_u0 = u0; m_v0 = v0; }

  // constant bottom default; optional planar slopes
  void SetBed(Real h0, Real Sx = Real(0), Real Sy = Real(0)) { m_h0 = h0; m_Sx = Sx; m_Sy = Sy; }

  void SetWind(Real ua, Real va) { m_ua = ua; m_va = va; }

  void SetBC(CasulliBC2D<Real> bc) { m_bc = bc; }
  ...
  static auto HfromEta(Real h, Real eta) noexcept -> Real { return std::max(Real(0), h + eta); }
...
private:
  void InstallNoOpHooks()
  {
    ...
    { out.assign(3, Real(0)); return out; });
    ...
    { out = vdot; if (out.size()!=3) out.resize(3, Real(0)); return out; };
